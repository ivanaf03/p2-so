...
int main(){
    ...
    do{
        ...
        processIn(trozos, comando,&list, &list_mem, &a);
    } while(a==1);
}
...
void processIn(char *trozos[], char *comando, tList *list, tListM *list_mem, int *a){ //CORRECTO, SE PUEDE CAMBIAR POR TABLA
    if(trozos[0]!=NULL){
        ...
        if(strcmp(trozos[0],"autores")==0){
            ...
            comEnd(trozos, comando, list, list_mem, a);
            ...
            comComandoN(trozos, list, list_mem, a);
...
void comEnd(char *trozos[], char *comando, tList *list, tListM *list_mem, int *a){
    if(trozos[1]==NULL){
        delete(list);
...
void comComandoN(char *trozos[], tList *list, tListM *list_mem, int *a){
    if(trozos[1]==NULL){
        for(int i=0;i<=last(*list);i++){
                printf("%d->%s\n",i, getItem(i,*list));
        }
        //Hace lo mismo que el comando hist
            ...
            if(x>=0 && x<last(*list)){
                ...
                if(strcmp(trozos[0], "comando")!=0){
                    ...
                    processIn(trozos,instruccion, list, list_mem, a);
